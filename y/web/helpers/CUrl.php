...
class CUrl
{
     ...
     */
    public static function cssFile($url,$media='screen')
    {
     ...
     * @param string $url URL for the JavaScript files
     * @return string the JavaScript file tag
     */
    public static function scriptFiles($uris)
    {
        $base_url = self::$sites['res'].'/js/'.self::getRequestHost().'/';
        $new_url = '';
        foreach($uris as $file){
            $new_url .= '<script type="text/javascript" src="'.self::encode($base_url.$file).'"></script>';
        }
        return $new_url;
    }
     ...
     * @return host fold name in the resourse server.
     */
    public static function getRequestHost(){
        $request_url = Yii::app()->request->hostInfo;
        foreach(self::$sites as $name => $site){
            if($request_url == $site){
                $host = $name;
            }
        }
